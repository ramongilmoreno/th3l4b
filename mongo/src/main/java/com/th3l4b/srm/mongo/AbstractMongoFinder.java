...
public abstract class AbstractMongoFinder implements IFinder {
	...
	protected abstract IMongoModelRuntime runtime () throws Exception;
	...
	public Collection<IInstance> all(String type) throws Exception {
		IMongoEntityRuntime mer = runtime().get(type);		
	...
	public IInstance find(IIdentifier id) throws Exception {
		IMongoEntityRuntime mer = runtime().get(id.getType());
		DBCollection collection = getDB().getCollection(mer.collection());
		DBObject found = collection.findOne(new BasicDBObject(IMongoConstants.FIELD_ID, id.getKey()));
	...
	public Collection<IInstance> references(IIdentifier id, String relationship)
		...
		IMongoEntityRuntime mer = runtime().get(rr.getSourceType());
		BasicDBObject of = new BasicDBObject(mer.get(rr.getField()).field(), id.getKey());
